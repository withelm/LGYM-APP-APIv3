...
[Generator]
public sealed class EnumsResxGenerator : IIncrementalGenerator
{
    private static readonly DiagnosticDescriptor ResxParseFailed = new(
        id: "LGYMRES003",
        title: "Failed to parse Enums.resx",
        messageFormat: "Failed to parse Enums.resx at '{0}': {1}",
        category: "Resources",
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var resxFiles = context.AdditionalTextsProvider
            .Where(file => file.Path.EndsWith("Enums.resx", StringComparison.OrdinalIgnoreCase));
        ...
        context.RegisterSourceOutput(resxContent, static (context, input) =>
        {
            if (string.IsNullOrWhiteSpace(input.Content))
            {
                ...
                context.AddSource("Enums.g.cs", GenerateSource(ImmutableArray<string>.Empty));
                return;
            }
            ...
            context.AddSource("Enums.g.cs", GenerateSource(keys));
        });
    }
    ...
    private static string GenerateSource(ImmutableArray<string> keys)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("public static class Enums");
        builder.AppendLine("{");
        builder.AppendLine("    private static readonly ResourceManager ResourceManager = new ResourceManager(\"LgymApi.Resources.Resources.Enums\", typeof(Enums).Assembly);");
        builder.AppendLine();
        builder.AppendLine("    public static string? GetString(string key, CultureInfo? culture = null)");
        builder.AppendLine("    {");
        builder.AppendLine("        return ResourceManager.GetString(key, culture ?? CultureInfo.CurrentUICulture);");
        builder.AppendLine("    }");
        builder.AppendLine();
        ...
        builder.AppendLine("public static class EnumTranslationKeys");
        builder.AppendLine("{");
        ...
        var grouped = keys
            .Select(k =>
            {
                ...
                return separatorIndex > 0
                    ? (Group: k.Substring(0, separatorIndex), Member: k.Substring(separatorIndex + 1), FullKey: k)
                    : (Group: (string?)null, Member: k, FullKey: k);
            })
        ...
        {
            builder.AppendLine($"    public static class {MakeIdentifier(group.Key)}");
            builder.AppendLine("    {");
            foreach (var entry in group)
            {
                builder.AppendLine($"        public const string {MakeIdentifier(entry.Member)} = \"{entry.FullKey}\";");
            }
            builder.AppendLine("    }");
            builder.AppendLine();
        }
