...
- Push delivery now persists `PushNotificationMessage` rows with payload metadata (`schemaVersion`, `type`, `eventId`, optional entity linkage), provider response summaries, retry timestamps, and failure classification; Infrastructure owns `FcmPushSender` and related `PushNotifications:Fcm:*` configuration only. Worker owns environment-selected scheduling.
