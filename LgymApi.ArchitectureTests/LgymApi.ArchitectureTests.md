...
- #391 guards keep Workout & Progress repositories and EF configurations module-owned, require its explicit public read/write contract surface for foreign modules, verify contract DI registrations are unique and owner-scoped, and preserve no-tracking/projection shapes for ranking and progress reads. The contract-only `#386` Reporting consumer remains unwired from production services.
