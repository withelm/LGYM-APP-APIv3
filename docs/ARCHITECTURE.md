...
The API runs with only `ConnectionStrings:Postgres`; `LGYM_MIGRATION_POSTGRES` remains restricted to offline DataSeeder/bootstrap commands. Every non-Testing host applies EF migrations through the validated `lgym_runtime` role, which owns the `public` schema and EF objects but remains `NOBYPASSRLS` with no elevated membership. Hangfire schema preparation remains offline-only.

The tutorial RLS pilot is a staging-only database containment layer for `UserTutorialProgresses` and `UserTutorialStepProgresses`. It supplements, never replaces, Application authorization. The Platform actor scope applies a parameterized transaction-local PostgreSQL actor setting on the service-owned transaction. Maintenance work owns role provisioning, Hangfire preparation, and operator-run policy activation or deactivation; it assumes the runtime role only for those table-owner DDL commands. Production retains disabled RLS expectations until a separately approved go/no-go changes the database and configuration together before traffic. See [`tutorial-rls-pilot.md`](security/tutorial-rls-pilot.md) for the canonical runbook, diagnostics, exit criteria, and rollback.
